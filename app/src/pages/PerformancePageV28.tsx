...
export function PerformancePageV28() {
  ...
  const [parcelChartMode, setParcelChartMode] = useState<"sortStatus" | "dwelled">("sortStatus");
          ...
          chart={showChart ? (
            <div>
              <div className="mb-4 inline-flex items-center rounded-button border border-line-hovered bg-white">
                <button type="button" onClick={() => setParcelChartMode("sortStatus")} className={cn("relative -my-px h-10 rounded-button px-6 text-body-md-strong transition-colors first:-ml-px last:-mr-px", parcelChartMode === "sortStatus" ? "z-10 bg-white text-ink ring-2 ring-inset ring-ink" : "text-ink-subdued hover:text-ink")}>Sort status</button>
                <button type="button" onClick={() => setParcelChartMode("dwelled")} className={cn("relative -my-px h-10 rounded-button px-6 text-body-md-strong transition-colors first:-ml-px last:-mr-px", parcelChartMode === "dwelled" ? "z-10 bg-white text-ink ring-2 ring-inset ring-ink" : "text-ink-subdued hover:text-ink")}>Dwelled parcels</button>
              </div>
              {parcelChartMode === "sortStatus" ? (
                <VolumeChart
                ...
                <VolumeChart
                  data={useAggregated ? aggregateDays(dwellChartData, payload.visibleDays, selectedLabel) : dwellChartData}
                  metric={metricConfigs.processed}
                  visibleDays={useAggregated ? undefined : payload.visibleDays}
                  seriesLabels={{ processed: "On time", sortedLate: "Late", lost: "Dwelled parcels", readyToSort: "Pending", forecasted: "Forecasted" }}
                  simpleLegend={{ color: "#df3480", label: "Dwelled parcels" }}
                />
              )}
            </div>
          ) : undefined}
