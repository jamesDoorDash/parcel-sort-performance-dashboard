      ...
      {card.delta && (
        isNeutral ? (
          ...
          <span className={cn("flex items-center gap-1", deltaTone)}>
            ...
            <span className="text-[14px] leading-[20px] font-bold">{card.delta.value}</span>
            <span className={cn("text-[14px] leading-[20px] font-normal", card.delta.tone === "negative" ? "text-negative" : "text-ink-subdued")}>{card.delta.tone === "positive" ? "above" : "below"} target</span>
          </span>
        )
      )}
    </div>
...
export function PerformancePageV21() {
