      ...
      {card.delta && (
        isNeutral ? (
          ...
          <span className={cn("mt-1 flex items-center gap-1 text-[14px] leading-[20px]", card.delta.tone === "negative" ? "font-bold text-negative" : "font-normal text-ink-subdued")}>
            ...
            {card.delta.value} {card.delta.tone === "positive" ? "above" : "below"} target
          </span>
        )
      )}
    </div>
...
export function PerformancePageV26() {
  const [view, setView] = useState<"facility" | "individuals">("facility");
  ...
  return (
    <div className="flex h-full flex-col overflow-y-scroll">
      <div className="mx-auto w-full max-w-[1220px] px-12 pt-12 pb-10">
      ...
      {/* Tab bar — full width */}
      <div className="relative mt-0.5 w-full border-b border-line">
        <div className="mx-auto w-full max-w-[1220px] px-12">
          <div className="flex items-center gap-6">
            <button
              type="button"
              onClick={() => setView("facility")}
              className={cn("relative px-0 py-2 text-body-md transition-colors", view === "facility" ? "font-medium text-ink" : "text-ink-subdued hover:text-ink")}
            >
              Facility
            ...
            <button
              ...
              className={cn("relative px-0 py-2 text-body-md transition-colors", view === "individuals" ? "font-medium text-ink" : "text-ink-subdued hover:text-ink")}
            >
              Individual workers
              {view === "individuals" && <span className="absolute inset-x-0 bottom-0 h-1 rounded-t-[4px] bg-ink" />}
            </button>
          </div>
        </div>
      </div>
