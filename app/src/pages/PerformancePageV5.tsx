...
type V5MetricId = V3MetricId | "parcelPreSortRate";
...
const V5_METRIC_ORDER: V5MetricId[] = [
  "parcelsProcessed",
  "parcelsSortedOnTime",
  "parcelsMissorted",
  "parcelsLost",
...
function getV5MetricDefinition(id: V3MetricId) {
...
function toV5Card(card: V3MetricCard): V3MetricCard {
...
export function PerformancePageV5() {
  const [range, setRange] = useState<DateRangeKey>("thisWeek");
  const [selectedMetric, setSelectedMetric] = useState<V5MetricId>("parcelsProcessed");
  ...
  const metric = selectedMetric === "parcelPreSortRate"
    ...
    : getV5MetricDefinition(selectedMetric);
          ...
          <div className="grid grid-cols-4 gap-4">
            {cards.map((card) => (
              <V3MetricSelectorCard
                ...
                onClick={() => setSelectedMetric(card.id as V5MetricId)}
              />
            ))}
          </div>
        </section>
