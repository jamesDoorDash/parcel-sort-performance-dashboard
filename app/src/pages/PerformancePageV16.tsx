...
function SectionKpiCard({ card }: { card: V3MetricCard }) {
  ...
  const isGood = !card.delta || isNeutral || card.delta.tone === "positive";
  const barColor = isPlaceholder || !card.delta ? "bg-line" : isGood ? "bg-positive" : "bg-negative";

  return (
    <div className="flex gap-3">
      <div className={cn("w-[3px] shrink-0 self-stretch rounded-full", barColor)} />
...
export function PerformancePageV16() {
